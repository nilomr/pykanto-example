...
# Acoustic similarity with neighbours: dispersal
## Model
```{r}


acsim_neighbours_disp_f <- brmsformula(
    mean_ac_sim ~ 1 +
        mean_dispersal + wytham_born + prop_immigrants + year + (1 | father),
    family = gaussian()
)

acsim_neighbours_disp_m <-
    brm(
        formula = acsim_neighbours_disp_f,
        data = data,
        # prior = priors,
        iter = 4000, warmup = 1000,
        chains = 4,
        ...
        backend = "cmdstanr",
        # sample_prior = "only",
        file = file.path(dirs$model_fits, "acsim_neighbours_disp_m"),
        file_refit = "on_change",
        control = list(adapt_delta = 0.99, max_treedepth = 15)
    )

brms::hypothesis(acsim_neighbours_disp_m, "mean_dispersal = 0", robust = TRUE)
parnames(acsim_neighbours_disp_m)
conditional_effects(acsim_neighbours_disp_m)
plot(acsim_neighbours_disp_m)
```
