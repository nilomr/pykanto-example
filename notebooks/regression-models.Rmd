...
conditional_effects(m_0, effects = "prop_immigrants")
conditional_effects(m_4)
conditional_effects(m_5)
...
expand_rast <- function(df, axis = 0, position = "first") {
    ...
    if (axis == 1) {
        tobind <- tibble(x = rep(xseq, each = length(ex$y)), y = rep(ex$y, times = length(xseq)))
    } else {
        tobind <- tibble(y = rep(xseq, each = length(ex$y)), x = rep(ex$x, times = length(xseq)))
    }
...
# subtract the next number from each number in the list uniquex:
margins <- list()
...
colticks <- round(seq(f = x_min, t = x_max, by = .4), digits = 1)

ggplot() +
    geom_sf(data = sf::st_as_sf(perimeter), color = NA, fill = "#616161") +
    geom_raster(data = masked_rast, aes(x = x, y = y, fill = estimate)) +
    # geom_point(
    #     data = data, aes(
    #         x = x, y = y,
    #         size = repertoire_size^2,
    #     ),
    #     pch = 21, fill = "white",
    ...
    theme(
        ...
        axis.ticks.y = element_blank(),
        axis.ticks.x = element_blank(),
        axis.text.x = element_text(colour = "#a3a3a3"),
        axis.text.y = element_text(colour = "#a3a3a3"),
    ...
    labs(
        x = NULL,
        y = NULL,
        title = "Spatial distribution of repertoire sizes",
        subtitle = "*Posterior conditional effects of spatial predictior. <br>
        Points represent individual repertoire sizes (range 1-9)*"
        # caption = "\nOther information"
    )
#     -> spatial_m_3
# ggsave(
#     filename = "spatial_m_3.jpg",
#     plot = spatial_m_3,
#     path = dirs$figs,
#     scale = 1,
#     width = 13,
