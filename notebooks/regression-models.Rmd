...
dyad_data <- dyad_data %>%  mutate(imm_status = wytham_born_1+ wytham_born_2)
...
m_6_df = as.data.frame(m_6_cond_effs$spatial_dist)
...
ggplot() +
    ...
    geom_sf(data = sf::st_as_sf(perimeter), fill = NA, colour = text_colour, size = 0.3) +
    # geom_point(
    #     data = data, aes(
    #         x = x, y = y
    #     ),
